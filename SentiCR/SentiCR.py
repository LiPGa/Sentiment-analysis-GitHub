...
def expand_contractions(s, contractions_dict=contractions_dict):
     ...
     return contractions_regex.sub(replace, s.lower().decode('utf-8'))
...
class SentiCR:
    def __init__(self, algo="GBT", training_data=None, load_from_disk=False):
        if load_from_disk:
    ...
    def load_model(self):
        with open(model_path, "rb") as model_f:
            classifier = pickle.load(model_f,  fix_imports=True)
        with open(vector_path, "rb") as vector_f:
            vectorizer = pickle.load(vector_f,  fix_imports=True)
    ...
    def create_model_from_training_data(self):
        ...
        # for 3000 data set
        first = True
        for row, sentiment_data in self.training_data.iterrows():
            if first:
                first = False
                continue
            try:
                comments = preprocess_text(str(sentiment_data.Text))
                training_comments.append(comments)
                training_ratings.append(sentiment_data.Polarity)
            except TypeError:
        ...
        smote_model = SMOTE(ratio='auto', random_state=None, k_neighbors=5, m_neighbors=15, out_step=.0001,
...
def get_classes_score(test_ratings, pred):
    unique, counts = np.unique(pred, return_counts=True)
    scores = precision_recall_fscore_support(test_ratings, pred, average=None, labels=unique)
...
def report(test_ratings, pred):
    ...
    print(precision_recall_fscore_support(test_ratings, pred, average='micro'))
    print(precision_recall_fscore_support(test_ratings, pred, average='macro'))
    print(get_classes_score(test_ratings, pred))
...
if __name__ == '__main__':
    ...
    dataset_file_path = "dataset_3000.csv"
    train_df, test_df = generate_train_test_sets(dataset_file_path)
    test_ratings, pred = validation(train_df, test_df)
    report(test_ratings, pred)
